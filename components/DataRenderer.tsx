...
let formatValueWithParamType = (paramType: ParamType, value: any, truncate: boolean, makeLink: boolean, labels?: Record<string, string>): JSX.Element => {
    ...
    if (paramType.baseType === 'address') {
        ...
        if (makeLink) {
            return <a href={`https://etherscan.io/address/${address}`} target={"_blank"}
    ...
    if (encoded.length > 96 && truncate) {
        encoded = encoded.substring(0, 48) + "..." + encoded.substring(encoded.length - 48);
    }
    return <>{encoded}</>;
}
...
export const DataRenderer = (props: DataRendererProps) => {
    ...
    try {
        ...
        let rendered = formatValueWithParamType(paramType, decodedData, truncate || false, makeLink, props.meta.labels);
