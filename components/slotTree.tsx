...
const SlotTreeItemContent = (props: SlotTreeItemContentProps) => {
    ...
    if (slotInfo.resolved) {
        content = <>{`name=`}<WithSeparator
            separator={<>,&nbsp;</>}>{Object.values(slotInfo.variables).map(v => v.fullName)}</WithSeparator>&nbsp;{`slot=${slot}`}</>;
    } else {
        content = <>{`slot=${slot}` + (slotInfo.type === 'mapping' || slotInfo.type === 'array' ? ` offset=${slotInfo.offset}` : '') + ` type=${slotInfo.type}`}</>
    }
...
export const renderSlotTree = (resolvedStorageSlots: Record<string, SlotInfo>, slot: string, path: string) => {
    ...
    if (slotInfo.type === 'mapping') {
        ...
        children.push(<TreeItem key={path} nodeId={path + ".key"}
    ...
    return <TreeItem
        ...
        label={<TreeItemContentSpan>
            <SlotTreeItemContent
                slot={slot}
                slotInfo={resolvedStorageSlots[slot]}/>
        </TreeItemContentSpan>}
    >
        {children}
    </TreeItem>;
