    ...
    let recursivelyRenderParams = (path: string, params: ParamType[], values: Result): JSX.Element[] => {
        return params.map((param, idx) => {
            ...
            if (param.baseType === 'tuple') {
                ...
                label = <>
                    {paramName}:&nbsp;<DataRenderer
                    meta={props.traceMetadata}
                    decodedData={value}
                    preferredType={param.format()}></DataRenderer>
                </>;
                children = [];
            }

            return <TreeItem key={nodeId} nodeId={nodeId} label={<TreeItemContentSpan>{label}</TreeItemContentSpan>}>
                {children}
            </TreeItem>;
        })
    }
