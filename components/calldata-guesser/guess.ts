...
const decodeWellFormedTuple = (
    ...
    bytes: Uint8Array,
    paramIdx: number,
    collectedParams: Array<ParamType | [number, number | null]>,
    ...
    if (paramOffset >= endOfStaticCalldata) {
        ...
        const finalParams = collectedParams.map((param) => {
            ...
            const dynamicDataStart =
                dynamicParams[dynamicParamIdx][0] + (dynamicParams[dynamicParamIdx][1] === null ? 0 : 32);
            ...
            if (maybeDynamicElementLen !== null) {
                ...
                debug(
                    `lhs=${countTrailingZeros(dynamicData.slice(lastWord, lastWord + 32))} rhs=${
    ...
    if (paramIdx === 0) {
        debug('backtracking');
        debug('input:');
        chunkString(ethers.utils.hexlify(bytes).substring(2), 64).forEach((v, i) =>
    ...
    if (maybeOffset !== null) {
        ...
        debug(
            `parameter ${
                paramOffset / 32
            } might be dynamic, got offset ${maybeOffset}, len ${maybeLength} ${isValidLength}, ${depth}, ${dynamicOnlyParams}`,
        );
