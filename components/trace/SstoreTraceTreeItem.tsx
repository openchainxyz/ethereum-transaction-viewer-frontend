...
export const SstoreTraceTreeItem = (props: SstoreTraceTreeItemProps) => {
    ...
    let dialogValues =
        ...
        >
            {vars.map((v, i) => {
                let oldDataRenderer = <DataRenderer meta={traceMetadata}
                                                    data={v.oldValue}
                                                    preferredType={v.type}/>
                let newDataRenderer = <DataRenderer meta={traceMetadata}
                                                    data={v.newValue}
                                                    preferredType={v.type}/>

                return <TreeItem key={i}
                                 nodeId={traceResult.txhash + "." + node.id + ".trace." + i}
                                 label={<TreeItemContentSpan>
                                     {v.name}:&nbsp;{oldDataRenderer}&nbsp;→&nbsp;{newDataRenderer}
                                 </TreeItemContentSpan>}/>;
            })}
        </TreeView>;
    ...
    let dialogTitle = <>
        sstore&nbsp;
        <WithSeparator separator={<>,&nbsp;</>}>{variablesSorted.map(v => {
    ...
    let dialogContent = <>
        <Grid container direction={"column"}>
            ...
            <Grid item>Slot: <code>{node.slot}</code></Grid>
            <Grid item>Old Value: <code>{node.oldValue}</code></Grid>
            <Grid item>New Value: <code>{node.newValue}</code></Grid>
            ...
            {vars.map((v, i) => {
                ...
                if (v.oldValue === v.newValue) {
                    return <span key={i} style={{
                        ...
                        {v.name}:&nbsp;{oldDataRenderer}&nbsp;→&nbsp;(unchanged)
                    </span>;
                } else {
                    return <span key={i} style={{display: 'flex'}}>
                        {v.name}:&nbsp;{oldDataRenderer}&nbsp;→&nbsp;{newDataRenderer}
                    </span>;
                }
            })}
